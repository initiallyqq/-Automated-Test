...
const scenario = {
  id: "tools-listing",
  name: "Tools listing endpoint returns structured tool metadata",
  route: "/api/v1/tools",
...
test.describe("Tools listing endpoint returns structured tool metadata", () => {
  ...
  // === Scenario flow ===
  test("Flow: Tools listing endpoint returns structured tool metadata", async ({ page, request }) => {
    // Navigation
    if (baseURL) {
      await page.goto(baseURL + "/api/v1/tools", { waitUntil: "networkidle", timeout: 15000 });
      await page.waitForTimeout(1000);
    }

    // Step 1: send GET /api/v1/tools
    await test.step("send GET /api/v1/tools", async () => {
      const apiPath = normalizePath("/api/v1/tools");
      const apiResp = await callApi(request, "GET", apiPath);
      if (apiResp) {
        expect.soft(apiResp.status(), "GET /api/v1/tools status").toBeLessThan(500);
      }
    });
    ...
    // Step 3: parse JSON and assert array length &gt; 0 and required fields present on first item
    await test.step("parse JSON and assert array length &gt; 0 and required fields present on first item", async () => {
      test.info().annotations.push({ type: "step", description: "parse JSON and assert array length &gt; 0 and required fields present on first item" });
    });

    await test.step("Assert: GET /api/v1/tools returns 200", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("GET /api/v1/tools returns 200", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: GET /api/v1/tools returns 200" : "UNVERIFIED: GET /api/v1/tools returns 200" });
    });

    await test.step("Assert: response is array of tool objects", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("response is array of tool objects", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: response is array of tool objects" : "UNVERIFIED: response is array of tool objects" });
    });

    await test.step("Assert: each tool has &#39;id&#39;, &#39;name&#39;, and &#39;description&#39; fields", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("each tool has 'id', 'name', and 'description' fields", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: each tool has &#39;id&#39;, &#39;name&#39;, and &#39;description&#39; fields" : "UNVERIFIED: each tool has &#39;id&#39;, &#39;name&#39;, and &#39;description&#39; fields" });
    });
