...
const scenario = {
  id: "workflow-graph-retrieval",
  name: "Workflow graph endpoint returns valid structure",
  route: "/api/v1/workflow/graph",
...
test.describe("Workflow graph endpoint returns valid structure", () => {
  ...
  // === Scenario flow ===
  test("Flow: Workflow graph endpoint returns valid structure", async ({ page, request }) => {
    // Navigation
    if (baseURL) {
      await page.goto(baseURL + "/api/v1/workflow/graph", { waitUntil: "networkidle", timeout: 15000 });
      await page.waitForTimeout(1000);
    }

    // Step 1: send GET /api/v1/workflow/graph
    await test.step("send GET /api/v1/workflow/graph", async () => {
      const apiPath = normalizePath("/api/v1/workflow/graph");
      const apiResp = await callApi(request, "GET", apiPath);
      if (apiResp) {
        expect.soft(apiResp.status(), "GET /api/v1/workflow/graph status").toBeLessThan(500);
      }
    });

    // Step 2: verify status 200
    await test.step("verify status 200", async () => {
      test.info().annotations.push({ type: "step", description: "verify status 200" });
    });

    // Step 3: parse JSON and assert &#39;nodes&#39; is present and non-empty
    await test.step("parse JSON and assert &#39;nodes&#39; is present and non-empty", async () => {
      test.info().annotations.push({ type: "step", description: "parse JSON and assert &#39;nodes&#39; is present and non-empty" });
    });

    await test.step("Assert: GET /api/v1/workflow/graph returns 200", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("GET /api/v1/workflow/graph returns 200", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: GET /api/v1/workflow/graph returns 200" : "UNVERIFIED: GET /api/v1/workflow/graph returns 200" });
    });

    await test.step("Assert: response contains non-empty nodes array", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("response contains non-empty nodes array", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: response contains non-empty nodes array" : "UNVERIFIED: response contains non-empty nodes array" });
    });

    await test.step("Assert: response conforms to expected workflow graph structure", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("response conforms to expected workflow graph structure", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: response conforms to expected workflow graph structure" : "UNVERIFIED: response conforms to expected workflow graph structure" });
    });
