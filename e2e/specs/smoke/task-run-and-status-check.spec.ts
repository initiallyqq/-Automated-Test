...
const scenario = {
  id: "task-run-and-status-check",
  name: "User triggers new task run and retrieves task status",
  route: "/api/v1/tasks/run",
...
test.describe("User triggers new task run and retrieves task status", () => {
  ...
  // === Scenario flow ===
  test("Flow: User triggers new task run and retrieves task status", async ({ page, request }) => {
    // Navigation
    if (baseURL) {
      await page.goto(baseURL + "/api/v1/tasks/run", { waitUntil: "networkidle", timeout: 15000 });
      await page.waitForTimeout(1000);
    }

    // Step 1: send POST /api/v1/tasks/run with minimal valid payload
    await test.step("send POST /api/v1/tasks/run with minimal valid payload", async () => {
      const apiPath = normalizePath("/api/v1/tasks/run");
      const apiResp = await callApi(request, "POST", apiPath);
      if (apiResp) {
        expect.soft(apiResp.status(), "POST /api/v1/tasks/run status").toBeLessThan(500);
      }
    });

    // Step 2: extract taskId from response
    await test.step("extract taskId from response", async () => {
      test.info().annotations.push({ type: "step", description: "extract taskId from response" });
    });

    // Step 3: send GET /api/v1/tasks/{taskId}
    await test.step("send GET /api/v1/tasks/{taskId}", async () => {
      const apiPath = normalizePath("/api/v1/tasks/{taskId}");
      const apiResp = await callApi(request, "GET", apiPath);
      if (apiResp) {
        expect.soft(apiResp.status(), "GET /api/v1/tasks/{taskId} status").toBeLessThan(500);
      }
    });

    // Step 4: verify response contains status &#39;pending&#39;
    await test.step("verify response contains status &#39;pending&#39;", async () => {
      test.info().annotations.push({ type: "step", description: "verify response contains status &#39;pending&#39;" });
    });

    await test.step("Assert: POST /api/v1/tasks/run returns 201", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("POST /api/v1/tasks/run returns 201", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: POST /api/v1/tasks/run returns 201" : "UNVERIFIED: POST /api/v1/tasks/run returns 201" });
    });

    await test.step("Assert: task ID is returned", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("task ID is returned", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: task ID is returned" : "UNVERIFIED: task ID is returned" });
    });

    await test.step("Assert: GET /api/v1/tasks/{taskId} returns task object with status &#39;pending&#39;", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("GET /api/v1/tasks/\\{taskId\\} returns task object with status 'pending'", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: GET /api/v1/tasks/{taskId} returns task object with status &#39;pending&#39;" : "UNVERIFIED: GET /api/v1/tasks/{taskId} returns task object with status &#39;pending&#39;" });
    });
