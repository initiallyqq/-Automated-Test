...
const scenario = {
  id: "auth-login-flow",
  name: "User logs in via API login endpoint",
  route: "/",
...
test.describe("User logs in via API login endpoint", () => {
  ...
  // === Scenario flow ===
  test("Flow: User logs in via API login endpoint", async ({ page, request }) => {
    // Navigation
    if (baseURL) {
    ...
    // Step 2: fill login form with demo credentials
    await test.step("fill login form with demo credentials", async () => {
      const filled = await safeFill(page, "login form with demo credentia", "Auto test login form with demo credentia");
    });

    // Step 3: submit form triggering POST /api/login
    await test.step("submit form triggering POST /api/login", async () => {
      const apiPath = normalizePath("/api/login");
      const apiResp = await callApi(request, "POST", apiPath);
      if (apiResp) {
        expect.soft(apiResp.status(), "POST /api/login status").toBeLessThan(500);
      }
    });

    // Step 4: verify navigation to / and presence of session cookie
    await test.step("verify navigation to / and presence of session cookie", async () => {
      test.info().annotations.push({ type: "step", description: "verify navigation to / and presence of session cookie" });
    });

    await test.step("Assert: login form submits successfully", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("login form submits successfully", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: login form submits successfully" : "UNVERIFIED: login form submits successfully" });
    });

    await test.step("Assert: redirects to / after POST /api/login", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("redirects to / after POST /api/login", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: redirects to / after POST /api/login" : "UNVERIFIED: redirects to / after POST /api/login" });
    });

    await test.step("Assert: session cookie is set", async () => {
      // Attempt to find assertion text on the page
      const found = await page.getByText(new RegExp("session cookie is set", "i")).first().isVisible({ timeout: 5000 }).catch(() => false);
      test.info().annotations.push({ type: "assertion", description: found ? "PASS: session cookie is set" : "UNVERIFIED: session cookie is set" });
    });
