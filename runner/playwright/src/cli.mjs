...
async function main() {
  ...
  if (!requestPath) {
    throw new Error("usage: node src/cli.mjs <request.json>");
  }

  const request = JSON.parse(stripBOM(await readFile(requestPath, "utf8")));
  ...
  const htmlReportDir = join(request.projectRoot, "artifacts", "playwright-report");
  const result = {
    exitCode: 0,
  ...
  const generatedSpecExists = generatedSpecs.length > 0;
  if (!generatedSpecExists) {
    result.exitCode = 1;
  }

  if (generatedSpecExists && await exists(playwrightCLI)) {
    try {
      const run = await runPlaywright({
        ...
        slowMoMs: Number(request.slowMoMs || 0)
      });
  ...
  result.htmlReportPath = (await exists(htmlReportDir)) ? htmlReportDir : undefined;
...
async function findGeneratedSpecs(root) {
  const specs = [];
...
function runPlaywright({ cliPath, projectRoot, config, outputDir, reportPath, specPattern, testResultsDir, headed, slowMoMs }) {
  return new Promise((resolveRun, rejectRun) => {
    ...
    child.stdout.on("data", chunk => {
      ...
      stdout += text;
      emitProgress("playwright_stdout", text);
    });
    child.stderr.on("data", chunk => {
      const text = chunk.toString();
      stderr += text;
      emitProgress("playwright_stderr", text);
    });
...
function emitProgress(type, text) {
  for (const line of text.split(/\r?\n/)) {
    if (!line.trim()) {
      continue;
    }
    process.stderr.write(JSON.stringify({ type, message: line }) + "\n");
  }
}
